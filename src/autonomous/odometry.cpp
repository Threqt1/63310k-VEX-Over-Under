...
void OdometryLoop(void *istate)
{
    ...
    while (true)
    {
        ...
        pros::lcd::print(0, "Y: %s", std::to_string(state->state.y.convert(inch)));
        pros::lcd::print(1, "X: %s", std::to_string(state->state.x.convert(inch)));
        pros::lcd::print(2, "Angle: %s", std::to_string(state->state.angle.convert(degree)));
